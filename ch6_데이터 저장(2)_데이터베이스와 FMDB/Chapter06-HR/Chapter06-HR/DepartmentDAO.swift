...
class DepartmentDAO {
    ...
    func find() -> [DepartRecord] {
        ...
        do {
            ...
            let sql = "SELECT depart_cd, depart_title, depart_addr FROM department ORDER BY depart_cd ASC;"
            ...
            while rs.next() {
                ...
                // append 메소드 호출 시 아래 튜플을 괄호없이 사용하지 않도록 주의
                departList.append( (Int(departCd), departTitle!, departAddr!) ) // 하나의 튜플
            }
    ...
    func get(departCd: Int) -> DepartRecord? {
        // 1. 질의 실행
        let sql = "SELECT depart_cd, depart_title, depart_addr FROM department WHERE depart_cd = ?;"
    ...
    func remove(departCd: Int) -> Bool {
        do {
            let sql = "DELETE FROME department WHERE depart_cd= ? ;"
