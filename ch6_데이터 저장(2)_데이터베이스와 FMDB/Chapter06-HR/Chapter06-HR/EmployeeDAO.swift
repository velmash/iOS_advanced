...
struct EmployeeVO {
    ...
    var empName = "" // 사원명
    var joinDate = "" // 입사일
    var stateCd = EmpStateType.ING // 재직 상태(기본값 : 재직중ㅇ)
...
class EmployeeDAO {
    ...
    lazy var fmdb: FMDatabase! = {
        ...
        // 2. 샌드박스 내 문저 디렉터리 경로에서 데이터베이스 파일을 읽어온다.
    ...
    func get(empCd: Int) -> EmployeeVO? {
        ...
        // 결과 집합 처리
        if let _rs = rs { // 결과 집합이 옵셔널 타입이므로, 바인딩 변수를 통해 옵셔널 해제된다.
            _rs.next()
    ...
    func create(param: EmployeeVO) -> Bool {
        do {
            ...
            print("Insert Error : \(errors.localizedDescription)")
            return false
        }
    }
